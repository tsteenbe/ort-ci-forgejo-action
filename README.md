  ...
  ort:
    ...
    steps:
      ...
      - name: Run Forgejo Action for ORT
        uses: oss-review-toolkit/ort-ci-forgejo-action@v1
```
    ...
    steps:
      ...
      - name: Run Forgejo Action for ORT
        ...
        with:
          ...
          run: 'cache-dependencies,config-git-use-https,analyzer,scanner,evaluator,advisor,reporter,upload-results'
          sw-name: 'Mime Types'
          sw-version: '2.1.35'
```

### Run only parts of the Forgejo Action for ORT

If you just want to install `ort` on your CI runner, you can change the `run` parameter to the following:
...
Instead of the latest released [ORT Docker image][ort-docker-image] you can set
